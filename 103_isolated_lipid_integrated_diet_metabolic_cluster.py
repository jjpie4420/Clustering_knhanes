# ============================================================
# 103_isolated_lipid_integrated_diet_metabolic_cluster.py
#
# Diet-metabolic integrated clustering within isolated lipid group
#
# Input features:
# - food-group z-score variables
# - metabolic variables: BMI, glucose, HbA1c, TG, HDL-C, SBP, DBP
#
# Important:
# This is NOT food-only dietary subtype.
# This is integrated dietary-metabolic phenotype clustering.
# ============================================================
...
df = df[
    df["exclusive_predisease_group"] == "isolated_lipid"
].copy()
...
for cl in profile.index:
    ...
    if lipid_risk_score > 0.6 and refined_score > prudent_score:
        label = "Refined-metabolic risk phenotype"

    elif prudent_score > refined_score and lipid_risk_score < 0:
        label = "Prudent-lower metabolic risk phenotype"

    elif lipid_risk_score > 0.6:
        label = "Metabolic-risk dominant phenotype"

    elif refined_score > prudent_score:
        label = "Refined dietary phenotype"

    else:
        label = "Intermediate phenotype"

    phenotype_labels = {
    1: "Prudent-lower metabolic risk phenotype",
    2: "Refined-metabolic risk phenotype",
}
    phenotype_labels[cl] = label

tmp["integrated_phenotype"] = tmp["integrated_cluster"].map(phenotype_labels)

print("\n[INTEGRATED PHENOTYPE LABELS]")
print(phenotype_labels)

# ============================================================
# MERGE FOOD-ONLY CLUSTER FOR COMPARISON
# ============================================================
...
if os.path.exists(FOOD_ONLY_CLUSTER):
    ...
    food_only_cols = [c for c in food_only_cols if c in food_only.columns]
    ...
    food_only = food_only.rename(columns=rename_map)
...
save_df = df.merge(
    ...
    ] + [c for c in ["food_only_cluster", "food_only_subtype"] if c in tmp.columns]],
    on="ID",
    how="left"
)
...
profile.to_csv(
    os.path.join(
        OUT_DIR,
        "103_integrated_feature_profile.csv"
    ),
    encoding="utf-8-sig"
)
...
food_profile.to_csv(
    os.path.join(
        OUT_DIR,
        "103_integrated_food_profile.csv"
    ),
    encoding="utf-8-sig"
)
...
metabolic_profile.to_csv(
    os.path.join(
        OUT_DIR,
        "103_integrated_metabolic_profile.csv"
    ),
    encoding="utf-8-sig"
)
...
explained.to_csv(
    os.path.join(
        OUT_DIR,
        "103_integrated_pca_explained.csv"
    ),
    index=False,
    encoding="utf-8-sig"
)
...
loadings.to_csv(
    os.path.join(
        OUT_DIR,
        "103_integrated_pca_loadings.csv"
    ),
    encoding="utf-8-sig"
)
...
plt.title("Integrated diet-metabolic phenotype clustering\nwithin isolated lipid group")
...
plt.savefig(
    os.path.join(
        OUT_DIR,
        "103_integrated_pca_cluster_plot.png"
    ),
    dpi=300,
    bbox_inches="tight"
)
...
plt.title("Food profile by integrated lipid phenotype")
...
plt.savefig(
    os.path.join(
        OUT_DIR,
        "103_integrated_food_profile_heatmap.png"
    ),
    dpi=300,
    bbox_inches="tight"
)
...
# ============================================================
# FIGURE 3 METABOLIC PROFILE HEATMAP
# ============================================================
...
plt.title("Metabolic profile by integrated lipid phenotype")
...
plt.savefig(
    os.path.join(
        OUT_DIR,
        "103_integrated_metabolic_profile_heatmap.png"
    ),
    dpi=300,
    bbox_inches="tight"
)
...
# ============================================================
# FIGURE 4 COMBINED FEATURE HEATMAP
# ============================================================
...
plt.title("Integrated diet-metabolic profile within isolated lipid group")
...
plt.savefig(
    os.path.join(
        OUT_DIR,
        "103_integrated_combined_profile_heatmap.png"
    ),
    dpi=300,
    bbox_inches="tight"
)
