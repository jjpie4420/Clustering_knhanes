# ============================================================
# 80_lipid_latent_foodpattern_cluster.py
#
# Dietary subtype clustering within borderline lipid population
# ============================================================
...
OUT_DIR = os.path.join(
    BASE_DIR,
    "results",
    "80_lipid_latent_foodpattern_cluster"
)
...
df["cluster"] = kmeans.fit_predict(X_pca[:, :2]) + 1
...
profile.index = [food_labels[x] for x in profile.index]
...
save_df.to_csv(
    os.path.join(
        OUT_DIR,
        "80_lipid_cluster_assigned_dataset.csv"
    ),
    index=False,
    encoding="utf-8-sig"
)
...
profile.to_csv(
    os.path.join(
        OUT_DIR,
        "80_lipid_cluster_food_profile.csv"
    ),
    encoding="utf-8-sig"
)
...
plt.title("Borderline lipid dietary subtype clustering")
plt.legend()
plt.tight_layout()
...
sns.heatmap(
    ...
    center=0,
    linewidths=0.5,
    cbar_kws={"label": "Mean food-group z-score"}
)

plt.title("Food-group profile by borderline lipid dietary subtype")
plt.xlabel("Cluster")
plt.ylabel("")
plt.tight_layout()
...
plt.savefig(
    os.path.join(
        OUT_DIR,
        "80_lipid_foodgroup_heatmap.png"
    ),
    dpi=300,
    bbox_inches="tight"
)
